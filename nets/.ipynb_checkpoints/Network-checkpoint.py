...
class Network(nn.Module):
    def __init__(self, num_classes, fp16=False, num_frame=10):
        ...
        self.motion_compensation_loss = nn.MSELoss()
    ...
    def forward(self, inputs): #4, 3, 5, 512, 512
        ...
        if self.training:
            ...
            motion = torch.cat([motion_feat[:,i,:,:,:] for i in range(self.num_frame)], dim=1)
            motion = self.mapping2(motion) 
            motion_loss = self.motion_compensation_loss(feat_F, motion)
